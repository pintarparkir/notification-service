# CI/CD setup — notification-service
...
| `SERVICE_URL_DEV` | optional | — | Public URL of the dev Cloud Run revision (e.g. `https://notification-service-dev-abc.a.run.app`). When set, the smoke job probes `/healthz`. |
...
REPO_NAME=notification-service        # Repository name
SA_NAME=notification-deployer
...
```bash
gcloud run deploy notification-service-dev \
...
gcloud run services describe notification-service-prod --region=$REGION \
  --format='value(status.traffic)'

# Point all traffic back at it
gcloud run services update-traffic notification-service-prod \
  --region=$REGION \
